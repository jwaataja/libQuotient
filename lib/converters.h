...
struct JsonConverter : _impl::JsonExporter<T> {
    ...
    // fromJson specialisation instead of specialising the entire JsonConverter
...
inline auto toSnakeCase(QString s)
{
    for (auto it = s.begin(); it != s.end(); ++it)
        if (it->isUpper()) {
            const auto offset = static_cast<int>(it - s.begin());
            s.insert(offset, '_'); // NB: invalidates iterators
            it = s.begin() + offset + 1;
            *it = it->toLower();
        }
    return s;
